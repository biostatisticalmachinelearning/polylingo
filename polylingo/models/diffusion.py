...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x_t: torch.Tensor, t: torch.Tensor, class_labels=None, cfg_scale: float = 1.0):
    ...
    @torch.no_grad()
    def p_sample(self, x_t: torch.Tensor, t: torch.Tensor, class_labels=None, cfg_scale: float = 1.0):
    ...
    @torch.no_grad()
    def sample(self, batch_size: int, class_labels=None, cfg_scale: float = 1.0, device=None, show_progress: bool = True):
        if device is None:
    ...
    @torch.no_grad()
    def sample_ddim(self, batch_size: int, class_labels=None, cfg_scale: float = 1.0,
                    ddim_steps: int = 50, eta: float = 0.0, device=None, show_progress: bool = True):
        if device is None:
            device = next(self.model.parameters()).device
    ...
    def training_loss(self, x_start: torch.Tensor, class_labels=None, noise=None) -> torch.Tensor:
